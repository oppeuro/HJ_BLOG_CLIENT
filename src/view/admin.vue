<template>
  <div class="admin">
    ...
    <transition name="content">
      <div v-show="content_box_show" class="content_flex_box"></div>
    </transition>
    <div class="edit_area">
      <!--v-if="edit_reset"-->
      <editor  ref="editor" @update_content_list="update_content_list" :edit_update_articleData="update_articleData"></editor>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    watch: {
      '$route': function (to, from) {
        if(!this.getCookie('HJ_BLOG_USER_')) {
...
<style lang="scss">
  ...
  .content_flex_box{position: relative; flex: 0 0 250px;width: 250px;}
